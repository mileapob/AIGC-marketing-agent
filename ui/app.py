...
def generate_content(product_desc: str, platform: str, reference_image):
    ...
    evaluation = result.get("evaluation_result", "暂无评估结果")

    return xhs, weibo, pyq, image_paths, status, keywords_str, evaluation
...
with gr.Blocks(title="AI营销内容生成助手", theme=gr.themes.Soft()) as demo:
    ...
    generate_btn.click(
        ...
        outputs=[xhs_output, weibo_output, pyq_output, image_gallery, status_box, trend_keywords, evaluation_output],
    )

    gr.Markdown(
        """
        ---
        **技术栈**：LangGraph · DeepSeek-V3 · DALL-E 3 · Qwen-VL-Max · MCP · ChromaDB · Tavily · MemorySaver
        """
    )
