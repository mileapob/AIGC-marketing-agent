...
# 原因：与编排逻辑耦合。现将其抽离为独立 Agent 文件，supervisor 只负责调用，
...
async def _call_mcp_save(scripts: list, image_paths: list, product_desc: str) -> dict:
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            save_tool = next(t for t in tools if "save" in t.name)
...
def node_save(state: GraphState) -> GraphState:
    ...
    result = asyncio.run(_call_mcp_save(
        scripts=state.get("scripts", []),
...
def node_evaluate(state: GraphState) -> GraphState:
    ...
    # **state是保留原有状态，**result是用新值覆盖/新增字段
    return {**state, **result}
# <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
